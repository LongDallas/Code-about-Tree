class Tree:
    class Position:
        def element(self):
            raise NotImplementedError('must beb implement by subclass')
        def __eq__(self,other):
            raise NotImplementedError('must be implement by subclass')
        def __ne__(self,other):
            return not (self == other)
    def root(self):
        raise NotImplementedError('mus be implement by subclass')
    def parent(self,p):
        raise NotImplementedError('must be implement by subclass')
    ...
    def _subtree_postorder(self,p):
        for c in self.children(p):
            for other in self._subtree_postorder(c):
                    yield other
            yield p
    def breathfirth(self):
        if not self.is_empty():
            finge = LinkedQueue()
            finge.enqueue(self.root())
            while not finge.is_empty():
                p = finge.dequeue()
                yield p
                for c in self.children(p): 
                    finge.enqueue(c)
...
class LinkedBinarytree(BinaryTree):
    ...
    def _delete(self,p):
        node = self._validate(p)
        if self.num_children(p) == 2 : raise ValueError("p has two child")
